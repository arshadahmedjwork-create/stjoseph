...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Check admin status - use service role to bypass RLS
    ...
    // Get submission ID from query params or request body
    const url = new URL(req.url);
    let submissionId = url.searchParams.get("id");
    
    // If not in query params, try to get from body (for POST requests from functions.invoke())
    if (!submissionId && req.method === "POST") {
      try {
        const body = await req.json();
        submissionId = body.id;
      } catch (e) {
        // Body parsing failed, continue with null submissionId
      }
    }
